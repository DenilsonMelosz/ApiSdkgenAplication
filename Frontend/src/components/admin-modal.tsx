...
export function AdminModal({ isOpen, onOpenChange }: AdminModalProps) {
  ...
  const handleResetPassword = async () => {
    if (!passwordResetUser || !api) return;

    setActionLoading(true);

    try {
      const generatedPassword = await api.generateNewPassword({
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        <DialogContent className="border-purple-200 bg-white sm:max-w-[800px]">
          ...
          {error && (
            <div className="rounded-md bg-red-50 p-4 text-red-800 border border-red-200 mb-4">
              {error}
            </div>
          )}
      ...
      >
        <AlertDialogContent className="border-purple-200 bg-white">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel
              className="border-purple-200 text-purple-800 hover:bg-purple-100"
            ...
            <div className="mb-4 mt-2">
              ...
              <div className="mt-1 flex gap-2">
                <Input
                  ...
                  readOnly
                  value={newPassword}
                  className="font-mono text-purple-800"
                />
                <Button
                  variant="outline"
                  size="icon"
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    toast({
                      title: "Senha copiada",
                      description:
                        "A nova senha foi copiada para a área de transferência.",
                    });
                  }}
                  className="border-purple-300 text-purple-700"
                >
              ...
              <p className="mt-2 text-xs text-purple-600">
                Atenção guarde esta senha em um local seguro. Ela não será
                mostrada novamente.
              </p>
            </div>
          )}
